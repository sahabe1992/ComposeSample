...
let HomepageMiddleNineStoriesAndFivePicsReducer = Reducer<HomepageMiddleNineStoriesAndFivePicsState, HomepageMiddleNineStoriesAndFivePicsAction, HomepageMiddleNineStoriesAndFivePicsEnvironment>.combine(
    ...
    CardItemReducer.forEach(state: \.storiesWithImage, action: /HomepageMiddleNineStoriesAndFivePicsAction.cardListItemActions(id: action:), environment: {
        environment in
        CardItemViewEnvironment(UpdateBookmark: environment.UpdateBookmark)
      }
    ),
    CardItemReducer.forEach(state: \.storiesWithoutImage, action: /HomepageMiddleNineStoriesAndFivePicsAction.cardListItemActions(id: action:), environment: {
        environment in
        CardItemViewEnvironment(UpdateBookmark: environment.UpdateBookmark)
      }
    ), .init{
    ...
    case .viewAppeared:
        ...
        state.storiesWithImage = IdentifiedArrayOf<CardItemState>(
            uniqueElements: [
                CardItemState(id: 0,
                                                                       imageUrl: "newyork",
        ...
        state.storiesWithoutImage = IdentifiedArrayOf<CardItemState>(
            uniqueElements: [
                CardItemState(id: 0,
                                                                       imageUrl: "newyork",
                                                                       title: "Former site of Mediacorp’s Caldecott Broadcast Centre sold for S$280.9 million",
